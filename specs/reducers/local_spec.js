  ...
  it('reduces correctly when there are previous variables in the store', () => {
    ...
    expect(reduce(state, statement)).toEqual(
      machine.build.state(
        ...
        machine.build.store(
          machine.build.equivalenceClass(lexical.number(10), machine.build.variable('y', 0)),
          machine.build.equivalenceClass(lexical.number(20), machine.build.variable('x', 0)),
          machine.build.equivalenceClass(lexical.number(30), machine.build.variable('x', 1)),
          machine.build.equivalenceClass(undefined, machine.build.variable('x', 2)),
        )
      )
    );
  });
