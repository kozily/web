...
const operator = noNamespacedBuiltIns["Wait"];

describe("The wait operator bult-in", () => {
  beforeEach(() => {
  ...
  describe("validation", () => {
    ...
    it("succeeds when the argument is a variable", () => {
      const args = Immutable.fromJS([
  ...
  describe("evaluation", () => {
    it("returns wait_condition when the argument is unbound", () => {
      const args = Immutable.fromJS([
        { value: undefined, variable: buildVariable("x", 0) },
      ]);
    ...
    it("returns variable when the argument is valid", () => {
      const args = Immutable.fromJS([
        {
          value: valueNumber(2),
      ...
      const evaluation = operator.evaluate(args);
      expect(evaluation.get("variable")).toEqual(buildVariable("x", 0));
      expect(evaluation.get("value")).toEqual(undefined);
      expect(evaluation.get("waitCondition")).toEqual(undefined);
    });

    it("returns value when the argument is valid", () => {
      const args = Immutable.fromJS([{ value: valueNumber(2) }]);
      const evaluation = operator.evaluate(args);
